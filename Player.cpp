...
void Player::updateMovement() {
    ...
    if (onGround && velocityX != 0) {
        if (++animTick >= 5) {  // Change frame every 5 updates
            ...
            QString path = QString(":/images/m%1%2.png")
                               .arg(animFrame + 1)
                               .arg(isright ? "R" : "L");
...
void Player::keyPressEvent(QKeyEvent *ev) {
    ...
    case Qt::Key_Up:
        jump();
        if (isright)
            setPixmap(QPixmap(":/images/mJ1R.png").scaled(60,60,Qt::KeepAspectRatio,Qt::SmoothTransformation));
        else
            setPixmap(QPixmap(":/images/mJ1L.png").scaled(60,60,Qt::KeepAspectRatio,Qt::SmoothTransformation));
        break;
